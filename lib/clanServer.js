...
async function getCurrentMembers(refresh = false) {
  ...
  if (refresh) {
    // Prefer Tracker Network when API key is set (PvE XP, player level, clan XP)
    if (getTrackerKey()) {
      try {
        const current = await getMembersFromTracker(config);
        if (current.length > 0 && current.some((m) => m.xp_clan != null || m.xp_pve != null || m.player_level != null)) {
          return { members: current, apiOk: true };
        }
      } catch (e) {
        // Fall through to TheDivisionTab
      }
    }
    try {
